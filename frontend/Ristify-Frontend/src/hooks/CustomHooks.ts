...
const useCustomQuery = <T extends Model>(
    ...
    const castedData: T | undefined = query.data as T;
    return {...query, setEnabledQuery, data: castedData};
    // return {...query, setEnabledQuery};
}
...
const useMediaController = () => {
    ...
    const onSongSlide = (newProgress: number): void => {
        ...
        currentSong.duration = audioRef.current!.duration;
    ...
    const onVolumeChange = (newVolume: number): void => {
        const normalisedVolume = newVolume / 100;

        setVolume(normalisedVolume);
        audioRef.current!.volume = normalisedVolume;
    }
    ...
    const onBack = () => {
        setSongsState({
            ...
            nextSong: currentSong // fetch new random song from source
        })
        currentSong.duration = audioRef.current!.duration;
        onSongSlide(0);
    }
